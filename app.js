...
//the base urlend point
const baseUrlEndPoint =`http://coronavirus-tracker-api.herokuapp.com/v2/locations`;
//const relevanceLabPlantData = `https://dl.ioconvert.com/document/2020070422024598ed4cf055024423a6ac9e3c2fc95146/plant_inventory-data-60c3cbfffa178855433ad575f9ebbe8ff.json`;
...
const countriesWithCountryCodes = {
  ...
  mapboxgl.accessToken = 'pk.eyJ1IjoiY29kZXBhaSIsImEiOiJja2M4MHg2OGsxNnBzMnJ0aXF5N3V1Z3ZzIn0.7rliG1sqEOn0DAmJxneGIw';
  let geocoder;
        ...
        features: await Promise.all(coronaData.locations.map( async location => {
            ...
            return{
                ...
                properties:{
                    ...
                    <tr>
                    <td>Longitude: </td>
                    <td>${location.latest.longitude}</td>
                </tr>
                        </tbody>
                    </table>
...
async function performAsyncCall(){
    ...
    //rl test
    //const rlResponse = await fetch(`${ relevanceLabPlantData}`);
   // const rlData = await rlResponse.json();
    //rltest end

    //console.log('wrwerqwr',data);
    const{latest,locations } = data;
    coronaData.latest = latest;
    coronaData.locations.push(...locations);

    //
          //  const{Country,Name} = rlData;
          //  rlPlantDetails.Country = Country;
          //  rlPlantDetails.Name = Name;
...
function renderUI(details,world=false){
    ...
    <table class="table">
        ...
        <td>${world ? details.confirmed : details.latest.deaths }</td>
        </tr>
        </tbody>
    </table>
...
function renderDetailsForSelectedLocation(event){
   ...
   //console.log("rtawtalemme",locationCoronaDetails);
